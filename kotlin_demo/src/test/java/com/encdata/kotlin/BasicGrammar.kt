...
class BasicGrammar {
     ...
     * 二、可变长参数函数
     */
    private fun changeLongParameter() {
        //可变长参数函数
     ...
     * lambda表达式使用实例：
     */
    private fun lambdaMethod() {
        val sumLambda: (Int, Int) -> Int = { x, y -> x + y }
        println("三、lambda(匿名函数):")
        println(sumLambda(3, 3))//输出6
    }

    /**
     * 四、定义常量和变量
     */
    private fun constantVarDefine() {
        /*定义常量和变量
        可变变量定义：var关键字
        var <标识符> : <类型> = <初始值>
    ...
    /**
     * 五、注释(与 Java 不同, Kotlin 中的块注释允许嵌套。)
     */
     ...
     * 八、类型检测及自动类型转换
     */
    private fun typeCheckAndParse() {
        println("八、类型检测及自动类型转换")
