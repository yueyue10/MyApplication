 ...
 */
class ObjectExpDeclare {
    /**
     * Kotlin 对象表达式(Expression)和对象声明(Declare)
     */
    ...
    /*引用该对象，我们直接使用其名称即可：DataProviderManager.registerDataProvider(……)*/

    /*当然你也可以定义一个变量来获取获取这个对象，当时当你定义两个不同的变量来获取这个对象时，
    你会发现你并不能得到两个不同的变量。也就是说通过这种方式，我们获得一个单例。*/

    private fun objectDeclare0(){
    ...
    private fun objectDeclare2() {
        var site = Site1()
        //  site.DeskTop.url // 错误，不能通过外部类的实例访问到该对象
        var url = Site1.DeskTop.url // 正确
        println(url)
    }
    //  对象可以有超类型：
