...
class ProxyDemo {
    ...
    /*定义一个被委托的类 该类需要包含 getValue() 方法和 setValue() 方法，
     ...
     * 四、把属性储存在映射中
     */
    private fun storePropertyInMap() {
        println("四、把属性储存在映射中")
        ...
        /*对于一个值可变(mutable)属性(也就是说,var 属性),
        除 getValue()函数之外,它的委托还必须 另外再提供一个名为setValue()的函数, 这个函数接受以下参数:*/
        //  property —— 必须是类型 KProperty<*> 或其超类型
     ...
     * 五、翻译规则
     */
    private fun localProxyProperty() {
        ...
        /*fun <T> MyUI1.bindResource(
                id: ResourceID<T>,
                propertyName: String
        ): ReadOnlyProperty<MyUI1, T> {
